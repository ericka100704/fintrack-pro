...
BASE = Path(r"c:\Users\ckayd\xammp\htdocs\deploy-main\deploy-main\assets\halloween")
...
def is_bg(r, g, b, a):
    ...
    if avg > 232 and sat < 40:
        return True
    if sat < 22 and 150 <= avg <= 250:
        return True
    # soft cream leftover
    if avg > 220 and sat < 30 and (r - b) < 35:
        return True
    return False
...
flood(BASE / "pumpkin.png", 256)
flood(BASE / "bat.png", 256)
flood(BASE / "ghost.png", 256)
# hero: strip outer white/cream only
hero = Image.open(BASE / "hero-scene.png").convert("RGBA")
bak = BASE / "hero-scene.bak.png"
if not bak.exists():
    hero.save(bak)
w, h = hero.size
pix = hero.load()
...
while stack:
    ...
    if not is_bg(r, g, b, a):
        continue
    # don't eat dark purple night sky as "bg" if it's intentional scene — only light frames
    avg = (r + g + b) / 3.0
    if avg < 160:
        continue
    vis[y][x] = True
...
print("hero", hero.size, hero.getpixel((0, 0)))
